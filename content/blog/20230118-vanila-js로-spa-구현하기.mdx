---
title: vanila-js로-spa-구현하기
date: '2023-01-18'
tags:
  - FE
  - vanila JS
category: dev
summary: 'Vanilla JS로 SPA 라우터를 직접 구현하며 배운 History API, 컴포넌트 설계, 에러 핸들링'
thumbnail: ''
published: true
---

![웹뷰](/blog/images/20230118-vanila-js로-spa-구현하기.jpeg)
...
![웹뷰](/blog/images/20230118-vanila-js로-spa-구현하기-01.png)
...
SPA 개발의 핵심 로직들은 `router.ts`에 있고 Class로 정의해 놓고 관리했다. 간단하게 Router Class는 `routes` : 페이지정보가 담긴 배열, `rootElement` : html의 진입점 `<main id='root'><main>`을 인자로 받고 내부 메서드들을 이용해 `history.pushState()`로 url이 바뀔때마다 해당 url에 매칭되는 페이지 컴포넌트를 렌더링 해준다.
...
**핵심은 url이 바뀔때마다 다른 페이지를 보여주면 된다는 것**이었다.
...
아무래도 DOM을 다시 그릴때 reflow, repaint를 거쳐 다시 조립을하고 요소를 끼워넣는데, 많은 요소들을 리렌더시키면 그만큼 속도, 자원도 많이 들것같다는 생각에 이렇게 구현했다.
